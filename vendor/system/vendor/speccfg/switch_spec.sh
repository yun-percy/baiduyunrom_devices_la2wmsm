...
strUninstallForOverlay=".overlayforuninstall"
strInstallForOverlay=".overlayforinstall"
strUninstallForOverlayLen="${#strUninstallForOverlay}"
strInstallForOverlayLen="${#strInstallForOverlay}"
strUninstallForSystemApk=".systemapkforuninstall"
strInstallForSystemApk=".systemapkforinstall"
strUninstallForSystemApkLen="${#strUninstallForSystemApk}"
strInstallForSystemApkLen="${#strInstallForSystemApk}"
...
installOverlayFunc()
{
  ...
  if [ "$softLink" -eq "0" ]
  then
    ln $srcPath $dstPath$strInstallForOverlay
  else
    ln -s $srcPath $dstPath$strInstallForOverlay
  fi
}

uninstallOverlayFunc()
{
  local srcPath=$1
  local dstPath=$2
  if [[ -f $dstPath ]] || [[ -h $dstPath ]]
  then
    echo "Will uninstall overlay" > $dstPath$strUninstallForOverlay
  fi
}

installSystemApkFunc()
{
  local srcPath=$1
  local dstPath=$2
  local softLink=$3
  ...
  if [ "$softLink" -eq "0" ]
  then
    ln $srcPath $dstPath$strInstallForSystemApk
  else
    ln -s $srcPath $dstPath$strInstallForSystemApk
  fi
}

uninstallSystemApkFunc()
{
  local srcPath=$1
  local dstPath=$2
  if [[ -f $dstPath ]] || [[ -h $dstPath ]]
  then
    echo "Will uninstall system apk" > $dstPath$strUninstallForSystemApk
  fi
}
...
installApkFunc()
{
  ...
  then
    if [[ "${dstPath:0:22}" == "/system/vendor/overlay" ]] || [[ "${dstPath:0:15}" == "/vendor/overlay" ]]
    then
      installOverlayFunc $srcPath $dstPath 1
    elif [[ "${dstPath:0:11}" == "/system/app" ]]
    then
      installSystemApkFunc $srcPath $dstPath 1
    else
...
uninstallApkFunc()
{
  ...
  then
    if [[ "${dstPath:0:22}" == "/system/vendor/overlay" ]] || [[ "${dstPath:0:15}" == "/vendor/overlay" ]]
    then
      uninstallOverlayFunc $srcPath $dstPath
    elif [[ "${dstPath:0:11}" == "/system/app" ]]
    then
      uninstallSystemApkFunc $srcPath $dstPath
    else
...
removeLocalSpec()
{
  ...
  local strNewSpec=$2
  getAllCarrier
  for carrier in ${arrCarrier[@]}
  do
    if [[ "$carrier" != "Default" ]] && [[ "$carrier" != "$strOldSpec" ]] && [[ "$carrier" != "$strNewSpec" ]]
    then
      mountFunc "/system/" "rw"
      rm -rf "$strVendorPath/$carrier"
      mountFunc "/system/" "ro"
    fi
  done
  getAllCarrier
...
checkUninstallOverlayDir()
{
  local curpath=$1
  echo "curpath=$curpath"
  for item in `ls -a $curpath/`
  do
    echo "checkUninstallOverlayDir $curpath/$item"
    if [ -f "$curpath/$item" ]
    then
      if [[ "${#item}" -gt "$strUninstallForOverlayLen" ]] && [[ "${item:((${#item}-$strUninstallForOverlayLen)):$strUninstallForOverlayLen}" == "$strUninstallForOverlay" ]]
      then
        if [ -f "$curpath/${item:0:((${#item}-$strUninstallForOverlayLen))}" ]
        then
          echo "rm $curpath/${item:0:((${#item}-$strUninstallForOverlayLen))}"
          rm "$curpath/${item:0:((${#item}-$strUninstallForOverlayLen))}"
        fi
        echo "rm $curpath/$item"
        rm "$curpath/$item"
      fi
    elif [ -d "$curpath/$item" ]
    then
      checkUninstallOverlayDir "$curpath/$item"
    fi
  done
}

checkInstallOverlayDir()
{
  local curpath=$1
  echo "curpath=$curpath"
  for item in `ls -a $curpath/`
  do
    echo "checkInstallOverlayDir $curpath/$item"
    if [ -f "$curpath/$item" ]
    then
      if [[ "${#item}" -gt "$strInstallForOverlayLen" ]] && [[ "${item:((${#item}-$strInstallForOverlayLen)):$strInstallForOverlayLen}" == "$strInstallForOverlay" ]]
      then
        echo "mv $curpath/$item" "$curpath/${item:0:((${#item}-$strInstallForOverlayLen))}"
        mv "$curpath/$item" "$curpath/${item:0:((${#item}-$strInstallForOverlayLen))}"
      fi
    elif [ -d "$curpath/$item" ]
    then
      checkInstallOverlayDir "$curpath/$item"
    fi
  done
}
...
checkOverlay()
{
  local strOverlayPath="/system/vendor/overlay"
  mountFunc "/system" "rw"
  checkUninstallOverlayDir $strOverlayPath
  checkInstallOverlayDir $strOverlayPath
  mountFunc "/system" "ro"
}

checkUninstallSystemApkDir()
{
  local curpath=$1
  echo "curpath=$curpath"
  for item in `ls -a $curpath/`
  do
    echo "checkSystemApkDir $curpath/$item"
    if [ -f "$curpath/$item" ]
    then
      if [[ "${#item}" -gt "$strUninstallForSystemApkLen" ]] && [[ "${item:((${#item}-$strUninstallForSystemApkLen)):$strUninstallForSystemApkLen}" == "$strUninstallForSystemApk" ]]
      then
        if [ -f "$curpath/${item:0:((${#item}-$strUninstallForSystemApkLen))}" ]
        then
          echo "rm $curpath/${item:0:((${#item}-$strUninstallForSystemApkLen))}"
          rm "$curpath/${item:0:((${#item}-$strUninstallForSystemApkLen))}"
        fi
        echo "rm $curpath/$item"
        rm "$curpath/$item"
        if [ -f "$curpath/${item:0:((${#item}-$strUninstallForSystemApkLen))}$strBakForReplace" ]
        then
          mv "$curpath/${item:0:((${#item}-$strUninstallForSystemApkLen))}$strBakForReplace" "$curpath/${item:0:((${#item}-$strUninstallForSystemApkLen))}"
        fi
      fi
    elif [ -d "$curpath/$item" ]
    then
      checkUninstallSystemApkDir "$curpath/$item"
    fi
  done
}

checkInstallSystemApkDir()
{
  local curpath=$1
  echo "curpath=$curpath"
  for item in `ls -a $curpath/`
  do
    ...
    if [ -f "$curpath/$item" ]
    then
      if [[ "${#item}" -gt "$strInstallForSystemApkLen" ]] && [[ "${item:((${#item}-$strInstallForSystemApkLen)):$strInstallForSystemApkLen}" == "$strInstallForSystemApk" ]]
      then
        if [ -f "$curpath/${item:0:((${#item}-$strInstallForSystemApkLen))}" ]
        then
          mv "$curpath/${item:0:((${#item}-$strInstallForSystemApkLen))}" "$curpath/${item:0:((${#item}-$strInstallForSystemApkLen))}$strBakForReplace"
        fi
        echo "mv $curpath/$item" "$curpath/${item:0:((${#item}-$strInstallForSystemApkLen))}"
        mv "$curpath/$item" "$curpath/${item:0:((${#item}-$strInstallForSystemApkLen))}"
      fi
    elif [ -d "$curpath/$item" ]
    then
      checkInstallSystemApkDir "$curpath/$item"
    fi
  done
}
...
checkSystemApk()
{
  local strSystemApkPath="/system/app"
  mountFunc "/system" "rw"
  checkUninstallSystemApkDir $strSystemApkPath
  checkInstallSystemApkDir $strSystemApkPath
  mountFunc "/system" "ro"
}
...
then
  ...
  then
    ...
    then
      ...
      removeLocalSpec $CurSpec $strSpec
      echo "" > "$strDataFolder/install_flag"
    fi
  else
    installSpec
